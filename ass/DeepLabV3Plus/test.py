...
def overlay_and_get_vis(rgb_array, nrg_array, mask_array,
    ...
    """
    Overlay the binary mask onto the original RGB and NRG images respectively to obtain the visualization results. Support channel-first (3,H,W) or channel-last (H,W,3) input.
    Args:
        rgb_array: HxWx3 or 3xHxW original RGB image (0-255 uint8 or 0-1 float)
        nrg_array: HxWx3 or 3xHxW original NRG image
        mask_array: HxW or HxWx1 binary mask (0/1 or True/False)
        color_rgb: color to overlay on the RGB image
        color_nrg: color to overlay on the NRG image
        alpha: transparency (0.0 - 1.0)
        return_img: if True, return (rgb_vis, nrg_vis)
        show: if True, call plt.show()
    Returns:
        (rgb_vis, nrg_vis) or None
    """
...
def plot_four_quadrants(rgb_gt, nrg_gt, rgb_pred, nrg_pred,
    ...
    """
    Display the 4 superimposed images in a 2×2 grid for easy comparison of the true values versus the predictions.
    Args:
        rgb_gt, nrg_gt — True mask overlaid RGB/NRG visualization (H, W, 3)
        rgb_pred, nrg_pred — Predicted mask overlaid RGB/NRG visualization
        titles — Tuple of titles for the four subplots
    """
...
def pltone(name):
    ...
    rgb_gt_vis, nrg_gt_vis = overlay_and_get_vis(
        ...
        color_rgb=(255, 0, 0), color_nrg=(0, 255, 255),
        alpha=0.5, return_img=True, show=False)
