...
def overlay_and_get_vis(rgb_array, nrg_array, mask_array,
    ...
    """
    Supports channel-first or channel-last input, and returns and/or displays the superimposed result. Args:
    rgb_array: Original RGB image of size HxWx3 or 3xHxW
    nrg_array: NRG false-color image of size HxWx3 or 3xHxW
    mask_array: Binary mask of size HxW or HxWx1
    return_img: Whether to return the overlaid image (rgb_vis, nrg_vis)
    show: Whether to call plt.show()
    """
    # 1) Make sure channel-last: (H,W,3)
    def to_chlast(img):
...
def pltone(name):
    ...
    state_dict = torch.load(
        r"E:\Downloads\BaiduNetdiskDownload\unet\[r, g, b, n, ndvi]-wubeilv\[r, g, b, n, ndvi]-wubeilv_checkpoint_epoch100.pth",
        map_location=device)
    ...
    # Real mask superimposition visualization
...
if __name__ == '__main__':
    ...
    # df.loc[len(df)] = ["g,ndvi-No magnification"] + test(r"E:\Downloads\BaiduNetdiskDownload\unet\[g,ndvi]-wubeilv\[g,ndvi]-wubeilv_checkpoint_epoch100.pth",2,1)
    # df.loc[len(df)] = ["r, g, b, n, ndvi-2.5 times"] + test(r"E:\Downloads\BaiduNetdiskDownload\unet\[r, g, b, n, ndvi]-2.5bei\[r, g, b, n, ndvi]-2.5bei_checkpoint_epoch100.pth",5,2)
    # df.loc[len(df)] = ["r, g, b, n, ndvi-No magnification"] + test(r"E:\Downloads\BaiduNetdiskDownload\unet\[r, g, b, n, ndvi]-wubeilv\[r, g, b, n, ndvi]-wubeilv_checkpoint_epoch100.pth",5,3)
    # df.loc[len(df)] = ["r, g, b, n-No magnification"] + test(r"E:\Downloads\BaiduNetdiskDownload\unet\[r, g, b, n]-wubeilv\[r, g, b, n]-wubeilv_checkpoint_epoch100.pth",4,4)
    # df.loc[len(df)] = ["r, g, n, ndvi-No magnification"] + test(r"E:\Downloads\BaiduNetdiskDownload\unet\[r, g, n, ndvi]-wubeilv\[r, g, n, ndvi]-wubeilv_checkpoint_epoch100.pth",4,5)
    # print(df)
    for i in random.sample([f for f in os.listdir(r"D:\workspace\unsw\9517\ass\USA_segmentation\resize\RGB_images\val")], 10):
        pltone(i)
