...
def dice_coeff(input: Tensor, target: Tensor, reduce_batch_first: bool = False, epsilon: float = 1e-6):
    ...
    # 如果是 [N,1,H,W]，就先 flatten 到 [N, H, W]
...
def dice_loss(input: Tensor, target: Tensor):
    # 我们这里输入 input 已经是 sigmoid 后的概率，target float
...
def main():
    ...
    while True:  # cur_itrs < opts.total_itrs:
        ...
        for batch in train_loader:
            ...
            bce = bce_criterion(outputs.squeeze(1), labels.float(),)  # labels 是 [N,1,H,W] float 0/1
            probs = torch.sigmoid(outputs.squeeze(1))  # 转成概率
            ...
            if (cur_itrs) % opts.val_interval == 0:
                ...
                if ret_samples:
                    ...
                    for s in ret_samples[:4]:
                        img, gt, pr = s
                        imgs.append(img[0:3, ...])  # 仅 RGB 3 通道
